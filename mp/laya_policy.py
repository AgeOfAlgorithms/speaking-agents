...
class NeedleMP:
    """Needle 3 (github.com/cactus-compute/needle) behind the same interface, set up as closely as its design allows.

    Needle is not a classifier: it is a small autoregressive tool-calling model (29-121M parameters) run by a
    native engine; text goes in, a JSON function call comes out, and a grammar compiled from the tool schemas
    makes the call well formed. So, to match what Laya and von are given:
      * same instruction and same text state, as the request text;
      * the actions possible RIGHT NOW are a closed enum on one tool, `choose_action` (their guide: closed sets
        as enums, few tools). The option list changes every step, so agents are cached per option list;
      * what `use` would do is dynamic, so it goes in the request text rather than in a cached schema;
      * speaking is a second tool, `say`, with the six slots as optional enums -- calling it takes the turn.
        (Needle decodes left to right, so later slots can depend on earlier ones, unlike zero-shot Laya/von.)
      * the conversation is reset every step: no memory beyond the state text, as for the other two.
    Its engine decodes greedily only, so compare it with the greedy Laya/von runs. It runs on the CPU, one
    decision at a time (no batching). The first call returned is the action; no call at all = wait.
    Anonymous telemetry is switched off."""

    def __init__(self, model=None, sample=False, mask=True, can_speak=True, **_):
        os.environ.setdefault("NEEDLE_TELEMETRY", "0")
        os.environ.setdefault("DO_NOT_TRACK", "1")
        import needle
        self._needle, self.mask, self.can_speak = needle, mask, can_speak
        self.agents, self.truncated, self.calls, self.refusals = {}, 0, 0, 0
        self.say_tool = {"name": "say", "description": prompts.SPEAK_TEXT + ". Fill only the words you need.",
                         "parameters": {"type": "object", "properties": {
                             slot: {"type": "string", "enum": [w for w in language.VOCAB[slot] if w]} for slot in language.SLOTS}}}

    def _agent(self, labels):
        key = tuple(labels)
        if key not in self.agents:
            described = "; ".join("%s = %s" % (l, prompts.describe(a, "use the tile you face (see use_would)"))
                                  for l, a in zip(labels, self._actions))
            tools = [{"name": "choose_action", "description": "Choose your next action in the game. " + described,
                      "parameters": {"type": "object", "properties": {"action": {"type": "string", "enum": list(labels)}},
                                     "required": ["action"]}}]
            self.agents[key] = self._needle.Needle(tools=tools + ([self.say_tool] if self.can_speak else []))
    ...
    def act_batch(self, percepts):
        ...
        for P in percepts:
            if P["downed"] or P["sleeping"]:
                out.append(("noop", None, None))
                continue
            _, offered = prompts.action_question(P, self.mask, can_speak=False)
            labels = [prompts.label(a) for a in offered]
            self._actions = offered
            agent = self._agent(labels)
            agent.reset()
            text = "%s\nuse_would: %s\nState: %s" % (prompts.INSTRUCTIONS.format(name=P["name"]), P["use"][0],
                                                    json.dumps(state_dict(P), separators=(",", ":")))
            r = agent.complete(text, max_new_tokens=96)
            self.calls += 1
            calls = r.get("function_calls") or r.get("suppressed_calls") or []
            action, speech = "noop", None
            if not calls:
                self.refusals += 1
            elif calls[0]["name"] == "say":
                args = calls[0].get("arguments", {})
                speech = tuple(language.VOCAB[s].index(args[s]) if args.get(s) in language.VOCAB[s] else 0 for s in language.SLOTS)
                speech = speech if any(speech) else None
            else:
                chosen = calls[0].get("arguments", {}).get("action")
                action = offered[labels.index(chosen)] if chosen in labels else "noop"
            picked = prompts.SPEAK if speech else prompts.label(action)
            shown = labels + ([prompts.SPEAK] if self.can_speak else [])
            out.append((action, speech, {"options": shown, "probs": [1.0 if l == picked else 0.0 for l in shown], "chose": picked,
                                         "said": language.render(speech) if speech else None, "confidence": r.get("confidence")}))
        return out
